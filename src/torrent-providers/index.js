...
async function scrapeGloDLS(query) {
  try {
    const html = await fetchHTML(`https://glodls.to/search_results.php?search=${encodeURIComponent(query)}`);
    if (!html) return [];
...
async function scrapeSolidTorrents(query) {
  try {
    const data = await fetchJSON(`https://solidtorrents.to/api/v1/search?q=${encodeURIComponent(query)}`);
...
async function scrapeLimeTorrents(query) {
  try {
    const html = await fetchHTML(`https://limetorrents.lol/search/all/${encodeURIComponent(query)}/`);
    if (!html) return [];
    const $ = cheerio.load(html);
    const results = [];
...
async function scrape1337x(query) {
  for (const base of x1337MIRRORS) {
    try {
      const html = await fetchHTML(`${base}/search/${encodeURIComponent(query)}/1/`);
      if (!html) continue;
      const $ = cheerio.load(html);
      const results = [];
...
async function scrapeEZTV(imdbId) {
  if (!imdbId) return [];
  try {
    const data = await fetchJSON(`https://eztv.re/api/get-torrents?imdb_id=${imdbId}`);
